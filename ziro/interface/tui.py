...
from ziro.interface.streaming_parser import parse_streaming_content
from ziro.interface.tool_components.agent_message_renderer import AgentMessageRenderer
from ziro.interface.tool_components.registry import get_tool_renderer
from ziro.interface.tool_components.user_message_renderer import UserMessageRenderer
from ziro.interface.utils import build_tui_stats_text
from ziro.llm.config import LLMConfig
from ziro.telemetry.tracer import Tracer, set_global_tracer
...
class ChatTextArea(TextArea):  # type: ignore[misc]
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._app_reference: ZiroTUIApp | None = None

    def set_app_reference(self, app: "ZiroTUIApp") -> None:
...
class SplashScreen(Static):  # type: ignore[misc]
    ...
    def _animate(self) -> None:
        ...
        if self._phase == self.PHASE_LOGO and self._tick >= self.LOGO_TICKS:
            ...
            self._phase = self.PHASE_FLASH
            self._tick = 0
        elif self._phase == self.PHASE_FLASH and self._tick >= self.FLASH_TICKS:
    ...
    def _render_frame(self) -> None:
        ...
        if self._phase == self.PHASE_LOGO:
            self._frame_logo(text)
        elif self._phase == self.PHASE_BOMB:
            self._frame_bomb(text, random)
        elif self._phase == self.PHASE_FLASH:
            self._frame_flash(text)
        elif self._phase == self.PHASE_EXPLODE:
            self._frame_explode(text, random)
        else:
    ...
    def _frame_logo(self, text: Text) -> None:
        ...
        for i, ch in enumerate(full):
            d = abs(i - shine)
            if d <= 1:
                text.append(ch, style=Style(color="#c084fc", bold=True))
            elif d <= 3:
                text.append(ch, style=Style(color=self.PRIMARY))
            elif d <= 5:
                text.append(ch, style=Style(color=self.ACCENT))
            else:
                text.append(ch, style=Style(color="#525252"))
        text.append("\n")
    ...
    def _frame_explode(self, text: Text, rng: Any) -> None:
        ...
        for line in self.BANNER_LINES:
            ...
            for ch in line:
                if ch == " ":
                    out.append(" ")
                elif rng.random() < progress:  # noqa: S311
                    out.append(" ")
                else:
                    r = rng.choice(debris)  # noqa: S311
                    c = rng.choice(colors)  # noqa: S311
                    out.append(r, style=Style(color=c, bold=True))
            text.append_text(out)
            text.append("\n")
        ...
        for _ in range(10):
            ...
            for _ in range(55):
                if rng.random() < particle_density:  # noqa: S311
                    ...
                    c = rng.choice(colors)  # noqa: S311
                    line.append(ch, style=Style(color=c, bold=rng.random() > 0.5))  # noqa: S311
                else:
                    line.append(" ")
        ...
        if progress > 0.4:
            ...
            g = int(85 * emerge)
            b = int(247 * emerge)
            text.append(f"\n         Ziro", style=Style(color=f"#{r:02x}{g:02x}{b:02x}", bold=True))
            text.append(f" v{self._version}\n", style=Style(dim=True))
    ...
    def _frame_done(self, text: Text) -> None:
        ...
        text.append("  ready\n", style=Style(color=self.PRIMARY, dim=True))


class HelpScreen(ModalScreen):  # type: ignore[misc]
    def compose(self) -> ComposeResult:
        yield Grid(
            Label("Ziro Help", id="help_title"),
            Label(
                "F1        Help\nCtrl+Q/C  Quit\nESC       Stop Agent\n"
...
class ZiroTUIApp(App):  # type: ignore[misc]
    ...
    def on_mount(self) -> None:
        ...
        self.set_timer(5.5, self._hide_splash_screen)
    ...
    def _merge_renderables(renderables: list[Any]) -> Text:
        ...
        for i, item in enumerate(renderables):
            if i > 0:
                combined.append("\n")
            ZiroTUIApp._append_renderable(combined, item)
        return ZiroTUIApp._sanitize_text(combined)
    ...
    def _append_renderable(combined: Text, item: Any) -> None:
        ...
        if isinstance(item, Text):
            combined.append_text(ZiroTUIApp._sanitize_text(item))
        elif isinstance(item, Group):
            for j, sub in enumerate(item.renderables):
                if j > 0:
                    combined.append("\n")
                ZiroTUIApp._append_renderable(combined, sub)
        else:
            inner = getattr(item, "renderable", None)
            if inner is not None:
                ZiroTUIApp._append_renderable(combined, inner)
            else:
    ...
    def _start_scan_thread(self) -> None:
        def scan_target() -> None:
            try:
                ...
                try:
                    agent = ZiroAgent(self.agent_config)
    ...
    def _send_user_message(self, message: str) -> None:
        ...
        try:
            ...
            with _graph_lock:
                agent_instance = _agent_instances.get(self.selected_agent_id)
    ...
    def action_confirm_stop_agent(self, agent_id: str) -> None:
        try:
            from ziro.tools.agents_graph.agents_graph_actions import stop_agent
    ...
    def _clean_copied_text(text: str) -> str:
        ...
        for line in lines:
            ...
            if stripped in ZiroTUIApp._DECORATIVE_LINES:
                continue
            if stripped and all(c == "─" for c in stripped):
                continue
            out = line
            for prefix in ZiroTUIApp._ICON_PREFIXES:
...
async def run_tui(args: argparse.Namespace) -> None:
    """Run ziro in interactive TUI mode with textual."""
    app = ZiroTUIApp(args)
    await app.run_async()
