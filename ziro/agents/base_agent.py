...
from ziro.llm import LLM, LLMConfig, LLMRequestFailedError
from ziro.llm.utils import clean_content
from ziro.runtime import SandboxInitializationError
from ziro.tools import process_tool_invocations
...
class AgentMeta(type):
    ...
    def __new__(cls, name: str, bases: tuple[type, ...], attrs: dict[str, Any]) -> type:
        ...
        prompt_dir = get_ziro_resource_path("agents", name)
...
class BaseAgent(metaclass=AgentMeta):
    ...
    def _add_to_agents_graph(self) -> None:
        from ziro.tools.agents_graph import agents_graph_actions
    ...
    async def _wait_for_input(self) -> None:
        ...
        if self.state.has_waiting_timeout():
            ...
            try:
                from ziro.tools.agents_graph.agents_graph_actions import _agent_graph, _graph_lock
        ...
        try:
            ...
            with _graph_lock:
                if agent_id not in _agent_messages:
                    return
                messages = list(_agent_messages.get(agent_id, []))
            if messages:
                ...
                for message in messages:
                    if not message.get("read", False):
                        ...
                        if sender_id == "user":
                            ...
                            # Compact inter-agent message envelope. The model
                            # already understands "received from another agent"
                            # from the tag name — repeating the instruction in
                            # delivery_notice/delivery_info every message wastes
                            # ~500 tokens × N messages per scan with no benefit.
                            msg_type = message.get("message_type", "information")
                            priority = message.get("priority", "normal")
                            message_content = (
                                f"<inter_agent_message from=\"{sender_name}\" "
