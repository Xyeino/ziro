...
class DockerRuntime(AbstractRuntime):
    ...
    def _find_available_port(self, exclude: set[int] | None = None) -> int:
        ...
        Retries up to 5 times to find a port not in the exclude set
        and not already in use by another process.
        """
        ...
        for _ in range(5):
            ...
            if port not in exclude:
                # Verify port is still free after releasing the socket
                try:
        ...
        raise SandboxInitializationError(
            ...
            "Could not find available ports after multiple attempts.",
        )
    ...
    def _wait_for_tool_server(self, max_retries: int = 30, timeout: int = 5) -> None:
        ...
        for attempt in range(max_retries):
            # Progressive backoff: 0.5s, 1s, 1.5s, 2s, ... capped at 5s
            wait_time = min((attempt + 1) * 0.5, 5.0)
            ...
            try:
                with httpx.Client(trust_env=False, timeout=timeout) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if server_status in ("healthy", "degraded"):
                            if server_status == "degraded":
                                logger.warning(
                                    "Tool server started in degraded mode: %s",
                                    data.get("checks", {}),
                                )
                            return
    ...
    def _create_container(self, scan_id: str, max_retries: int = 2) -> Container:
        container_name = f"ziro-scan-{scan_id}"
        image_name = Config.get("ziro_image")
        if not image_name:
            raise ValueError("ZIRO_IMAGE must be configured")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                self._caido_port = self._find_available_port(
                    exclude={self._tool_server_port}
                )
                ...
                container = self.client.containers.run(
                    ...
                    environment={
                        ...
                        "ZIRO_SANDBOX_EXECUTION_TIMEOUT": str(execution_timeout),
    ...
    def _get_or_create_container(self, scan_id: str) -> Container:
        ...
        try:
            containers = self.client.containers.list(
                all=True, filters={"label": f"ziro-scan-id={scan_id}"}
            )
            if containers:
    ...
    def _copy_local_directory_to_container(
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "Failed to copy directory '%s' to container: %s", local_path, e
            )
    ...
    async def _register_agent(self, api_url: str, agent_id: str, token: str) -> None:
        try:
            ...
            logging.getLogger(__name__).warning(
                "Failed to register agent '%s' with tool server: %s", agent_id, e
            )
