...
from ziro.telemetry import posthog
from ziro.telemetry.flags import is_otel_enabled
from ziro.telemetry.utils import (
    TelemetrySanitizer,
    append_jsonl_record,
    bootstrap_otel,
    format_span_id,
...
class Tracer:
    ...
    def _setup_telemetry(self) -> None:
        ...
        (
            ...
            tracer_name="ziro.telemetry.tracer",
        )
    ...
    def _emit_event(
        ...
        if self._otel_tracer is not None:
            try:
                with self._otel_tracer.start_as_current_span(
                    ...
                    span.set_attribute("ziro.event_type", event_type)
                    span.set_attribute("ziro.source", source)
                    span.set_attribute("ziro.run_id", self.run_id)
                    span.set_attribute("ziro.run_name", self.run_name or "")

                    if status:
                        span.set_attribute("ziro.status", status)
        ...
        # Broadcast to live dashboard if running
        try:
            from ziro.dashboard.server import get_dashboard

            dashboard = get_dashboard()
            if dashboard:
                dashboard.broadcast(event_type, sanitized_payload or {}  )
        except ImportError:
            pass
    ...
    def set_run_name(self, run_name: str) -> None:
        ...
        self._run_completed_emitted = False
        # Reset saved vuln IDs so reports get re-saved to the new directory
    ...
    def get_run_dir(self) -> Path:
        if self._run_dir is None:
            ...
            try:
                from ziro.evidence import EvidenceCollector, set_evidence_collector

                collector = EvidenceCollector(self._run_dir)
                set_evidence_collector(collector)
            except ImportError:
                pass
    ...
    def add_vulnerability_report(  # noqa: PLR0912
        ...
        # Dedup check
        try:
            from ziro.scope.dedup import is_duplicate

            is_dup, existing_id = is_duplicate(
                self.vulnerability_reports,
                new_title=title,
                new_severity=severity,
                new_target=target or "",
                new_endpoint=endpoint or "",
            ...
            if is_dup and existing_id:
                logger.info("Skipping duplicate vulnerability: '%s' (matches %s)", title, existing_id)
                return existing_id
        except ImportError:
            pass
...
"""
    ...
    def set_scan_config(self, config: dict[str, Any]) -> None:
        ...
        # Initialize scope guard from targets
        try:
            from ziro.scope import ScopeGuard, set_scope_guard

            targets = config.get("targets", [])
            if targets:
                guard = ScopeGuard(targets)
                set_scope_guard(guard)
                logger.info("Scope guard initialized with %d target(s)", len(targets))
        except ImportError:
            pass

    def _save_vulnerability_file(self, vuln_dir: Path, report: dict[str, Any]) -> bool:
        """Save a single vulnerability report to a markdown file. Returns True on success."""
        try:
            ...
            logger.exception("Failed to save vulnerability report %s", report.get("id"))
            return False
    ...
    def save_run_data(self, mark_complete: bool = False) -> None:
        ...
        # Save final penetration test report
        if self.final_scan_result:
            try:
                ...
                logger.exception("Failed to save penetration test report")

        # Save individual vulnerability reports (each in its own try/except)
        if self.vulnerability_reports:
            try:
                vuln_dir = run_dir / "vulnerabilities"
                vuln_dir.mkdir(exist_ok=True)
            except OSError:
                logger.exception("Failed to create vulnerabilities directory")
                return
            ...
            for report in new_reports:
                if self._save_vulnerability_file(vuln_dir, report):
            ...
            try:
                ...
                logger.exception("Failed to save vulnerability CSV index")
