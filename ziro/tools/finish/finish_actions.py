...
def finish_scan(
    ...
    try:
        ...
        if tracer:
            ...
            # Generate reproduce scripts and evidence index
            reproduce_path = None
            evidence_index = None
            try:
                from ziro.evidence.reproduce import generate_reproduce_script

                run_dir = tracer.get_run_dir()
                reproduce_path = generate_reproduce_script(
                    tracer.vulnerability_reports, run_dir, format="python"
                )
                generate_reproduce_script(
                    tracer.vulnerability_reports, run_dir, format="shell"
                )
            ...
            try:
                ...
                if collector:
                    evidence_index = collector.generate_evidence_index()
            except (ImportError, OSError):
                pass
            ...
            if reproduce_path:
                result["reproduce_script"] = str(reproduce_path)
            if evidence_index:
                result["evidence_index"] = str(evidence_index)
            return result
