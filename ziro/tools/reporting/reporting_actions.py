...
def _is_blackbox_scan() -> bool:
    """Check if the current scan is black-box only (no source code targets)."""
    try:
        ...
        if tracer and tracer.scan_config:
            ...
            source_types = {"repository", "local_code"}
            return not any(t.get("type") in source_types for t in targets)
...
def create_vulnerability_report(  # noqa: PLR0912
    ...
    if parsed_locations:
        if _is_blackbox_scan():
            # In black-box mode, source code is not available — code_locations are hallucinated
            import logging

            logging.getLogger(__name__).warning(
                "Stripping code_locations from report '%s': "
                "black-box scan has no source code access, "
                "code analysis would be fabricated.",
                title,
            )
            blackbox_warning = (
                "code_locations were provided but stripped: "
                "this is a black-box scan with no source code access. "
    ...
    try:
        ...
        tracer = get_global_tracer()
        if tracer:
            from ziro.llm.dedupe import check_duplicate
