...
if os.getenv("ZIRO_SANDBOX_MODE", os.getenv("ZIRO_SANDBOX_MODE", "false")).lower() == "false":
    from ziro.runtime import get_runtime
...
_SERVER_TIMEOUT = float(Config.get("ziro_sandbox_execution_timeout") or "120")

# Tools whose arguments may contain URLs/hosts that should be scope-checked
_SCOPE_CHECKED_ARGS = {
...
def _check_scope(tool_name: str, kwargs: dict[str, Any]) -> str | None:
    """Check if tool arguments target URLs/hosts within the allowed scope.

    Returns an error string if out of scope, None if OK.
    """
    try:
    ...
    arg_names = _SCOPE_CHECKED_ARGS.get(tool_name)
    if not arg_names:
        return None
    ...
    for arg_name in arg_names:
        ...
        for url in urls:
            if not guard.check_and_log(url, tool_name):
                return (
                    f"OUT OF SCOPE: {url} is not in the allowed target list. "
                    f"Allowed targets: {', '.join(guard.summary()['allowed_domains'] + guard.summary()['allowed_ips'])}. "
                    f"Do not attempt to interact with targets outside the defined scope."
                )
...
SANDBOX_CONNECT_TIMEOUT = float(Config.get("ziro_sandbox_connect_timeout") or "10")


async def execute_tool(tool_name: str, agent_state: Any | None = None, **kwargs: Any) -> Any:
    execute_in_sandbox = should_execute_in_sandbox(tool_name)
    sandbox_mode = os.getenv("ZIRO_SANDBOX_MODE", os.getenv("ZIRO_SANDBOX_MODE", "false")).lower() == "true"
...
async def _execute_tool_locally(tool_name: str, agent_state: Any | None, **kwargs: Any) -> Any:
    ...
    if needs_agent_state(tool_name):
        ...
        call = lambda: tool_func(agent_state=agent_state, **converted_kwargs)  # noqa: E731
    else:
        call = lambda: tool_func(**converted_kwargs)  # noqa: E731

    if inspect.iscoroutinefunction(tool_func):
        return await call()

    # Run sync tool functions in a thread to avoid blocking the event loop
...
async def execute_tool_with_validation(
    ...
    scope_error = _check_scope(tool_name, kwargs)
    if scope_error:
