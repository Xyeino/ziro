...
def _process_dynamic_content(content: str) -> str:
    if "{{DYNAMIC_SKILLS_DESCRIPTION}}" in content:
        try:
            from ziro.skills import generate_skills_description
...
def _get_schema_path(func: Callable[..., Any]) -> Path | None:
    ...
    return get_ziro_resource_path("tools", folder, schema_file)


def _is_sandbox_mode() -> bool:
    return os.getenv("ZIRO_SANDBOX_MODE", os.getenv("ZIRO_SANDBOX_MODE", "false")).lower() == "true"


def _is_browser_disabled() -> bool:
    if os.getenv("ZIRO_DISABLE_BROWSER", os.getenv("ZIRO_DISABLE_BROWSER", "")).lower() == "true":
        return True

    from ziro.config import Config

    val: str = Config.load().get("env", {}).get("ZIRO_DISABLE_BROWSER", "")
