...
def resolve_ziro_model(model_name: str | None) -> tuple[str | None, str | None]:
    """Resolve a ziro/ model into names for API calls and capability lookups.

    Returns (api_model, canonical_model):
    - api_model: openai/<base> for API calls (Ziro API is OpenAI-compatible)
    - canonical_model: actual provider model name for litellm capability lookups
    Non-ziro models return the same name for both.
    """
    if not model_name or not model_name.startswith("ziro/"):
        return model_name, model_name

    base_model = model_name[6:]
    api_model = f"openai/{base_model}"
    canonical_model = ZIRO_MODEL_MAP.get(base_model, api_model)
