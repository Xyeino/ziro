---
title: "Configuration"
description: "Environment variables for Ziro"
---

Configure Ziro using environment variables or a config file.

## LLM Configuration

<ParamField path="ZIRO_LLM" type="string" required>
...
<ParamField path="ZIRO_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="ZIRO_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="ZIRO_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="ZIRO_DISABLE_BROWSER" default="false" type="boolean">
  Disable browser automation tools.
</ParamField>

<ParamField path="ZIRO_TELEMETRY" default="1" type="string">
...
<ParamField path="ZIRO_OTEL_TELEMETRY" type="string">
  Enable/disable OpenTelemetry run observability independently. When unset, falls back to `ZIRO_TELEMETRY`.
</ParamField>

<ParamField path="ZIRO_POSTHOG_TELEMETRY" type="string">
  Enable/disable PostHog product telemetry independently. When unset, falls back to `ZIRO_TELEMETRY`.
</ParamField>

<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Ziro keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Ziro still writes complete run telemetry locally to:

```bash
ziro_runs/<run_name>/events.jsonl
```

When remote vars are set, Ziro dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="ZIRO_IMAGE" default="ghcr.io/Xyeino/ziro-sandbox:0.1.13" type="string">
...
<ParamField path="ZIRO_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="ZIRO_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="ZIRO_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Ziro stores configuration in `~/.ziro/cli-config.json`. You can also specify a custom config file:

```bash
ziro --target ./app --config /path/to/config.json
```
...
export ZIRO_SANDBOX_EXECUTION_TIMEOUT="300"
