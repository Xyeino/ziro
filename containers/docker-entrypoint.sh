...
export ZIRO_SANDBOX_MODE=true
export POETRY_VIRTUALENVS_CREATE=false
export TOOL_SERVER_TIMEOUT="${ZIRO_SANDBOX_EXECUTION_TIMEOUT:-${ZIRO_SANDBOX_EXECUTION_TIMEOUT:-120}}"
TOOL_SERVER_LOG="/tmp/tool_server.log"

if [ -z "$TOOL_SERVER_PORT" ]; then
  echo "ERROR: TOOL_SERVER_PORT environment variable is not set or empty."
  exit 1
fi

if [ -z "$TOOL_SERVER_TOKEN" ]; then
  echo "ERROR: TOOL_SERVER_TOKEN environment variable is not set or empty."
  exit 1
fi

sudo -E -u pentester \
  poetry run python -m ziro.runtime.tool_server \
