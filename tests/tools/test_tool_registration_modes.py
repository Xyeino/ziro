...
from ziro.tools.registry import clear_registry
...
def _reload_tools_module() -> ModuleType:
    ...
    for name in list(sys.modules):
        if name == "ziro.tools" or name.startswith("ziro.tools."):
            sys.modules.pop(name, None)

    return importlib.import_module("ziro.tools")


def test_non_sandbox_registers_agents_graph_but_not_browser_or_web_search_when_disabled(
    monkeypatch: Any,
) -> None:
    monkeypatch.setenv("ZIRO_SANDBOX_MODE", "false")
    monkeypatch.setenv("ZIRO_DISABLE_BROWSER", "true")
...
def test_sandbox_registers_sandbox_tools_but_not_non_sandbox_tools(
    ...
    monkeypatch.setenv("ZIRO_SANDBOX_MODE", "true")
    monkeypatch.setenv("ZIRO_DISABLE_BROWSER", "true")
    monkeypatch.delenv("PERPLEXITY_API_KEY", raising=False)
    monkeypatch.setattr(Config, "load", classmethod(_empty_config_load))
...
def test_load_skill_import_does_not_register_create_agent_in_sandbox(
    ...
    load_skill_module = importlib.import_module("ziro.tools.load_skill.load_skill_actions")
    registry = importlib.import_module("ziro.tools.registry")
