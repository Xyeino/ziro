...
from ziro.telemetry import tracer as tracer_module
from ziro.telemetry import utils as telemetry_utils
...
def _reset_tracer_globals(monkeypatch) -> None:
    ...
    monkeypatch.delenv("ZIRO_TELEMETRY", raising=False)
    monkeypatch.delenv("ZIRO_OTEL_TELEMETRY", raising=False)
    monkeypatch.delenv("ZIRO_POSTHOG_TELEMETRY", raising=False)
...
def test_tracer_local_mode_writes_jsonl_with_correlation(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "local-observability" / "events.jsonl"
...
def test_tracer_redacts_sensitive_payloads(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "redaction-run" / "events.jsonl"
    events = _load_events(events_path)
...
def test_tracer_remote_mode_configures_traceloop_export(monkeypatch, tmp_path) -> None:
    ...
    assert "ziro.run_id" not in init_kwargs["resource_attributes"]
    assert "ziro.run_name" not in init_kwargs["resource_attributes"]

    events_path = tmp_path / "ziro_runs" / "remote-observability" / "events.jsonl"
    events = _load_events(events_path)
...
def test_run_completed_event_emitted_once(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "single-complete" / "events.jsonl"
    events = _load_events(events_path)
...
def test_events_with_agent_id_include_agent_name(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "agent-name-enrichment" / "events.jsonl"
    events = _load_events(events_path)
...
def test_run_metadata_is_only_on_run_lifecycle_events(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "metadata-scope" / "events.jsonl"
    events = _load_events(events_path)
...
def test_set_run_name_resets_cached_paths(monkeypatch, tmp_path) -> None:
    ...
    assert new_events_path == tmp_path / "ziro_runs" / "renamed-run" / "events.jsonl"
...
def test_set_run_name_resets_run_completed_flag(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "renamed-complete" / "events.jsonl"
    events = _load_events(events_path)
    run_completed = [event for event in events if event["event_type"] == "run.completed"]
...
def test_events_write_locks_are_scoped_by_events_file(monkeypatch, tmp_path) -> None:
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ZIRO_TELEMETRY", "0")
...
def test_tracer_skips_jsonl_when_telemetry_disabled(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "telemetry-disabled" / "events.jsonl"
...
def test_tracer_otel_flag_overrides_global_telemetry(monkeypatch, tmp_path) -> None:
    ...
    events_path = tmp_path / "ziro_runs" / "otel-enabled" / "events.jsonl"
    assert events_path.exists()
