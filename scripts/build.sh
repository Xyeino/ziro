...
echo -e "${BLUE}⚡ Ziro Build Script${NC}"
echo "================================"
...
poetry run pyinstaller ziro.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="ziro-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/ziro.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/ziro.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    cp "dist/ziro" "$RELEASE_DIR/$BINARY_NAME"
